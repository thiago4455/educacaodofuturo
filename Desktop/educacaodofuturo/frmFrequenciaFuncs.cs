...
namespace educacaodofuturo
{
    public partial class frmFrequenciaFuncs : Form
    {
        ...
        {
            InitializeComponent();
            new Resources.Funcionario().RetFreqData("data",DateTime.Now.ToString("dd/MM/yyyy"),ResultFrequencia);
        ...
        private void btnPesquisar_Click(object sender, EventArgs e)
        {
            ...
            if(cboOpcao.SelectedIndex == 0)
            {
                if (DateTime.TryParse(mskPesquisar.Text, out temp))
                    new Resources.Funcionario().RetFreqData("data", mskPesquisar.Text, ResultFrequencia);
                else
                    MessageBox.Show("Data inválida!", "Erro");
            }
            else
            {
                new Resources.Funcionario().RetFreqData("cpf", mskPesquisar.Text, ResultFrequencia);
            }
        }
    }
}
